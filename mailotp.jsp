...
<%
    ...
    Connection cona = DriverManager.getConnection(uRl,useRName,paR);
    Statement s1=cona.createStatement();
    ResultSet re=s1.executeQuery("select * from customer");
    boolean flag=false;
    while(re.next()){
        if(re.getString("email").equals(to)){
            flag=true;
            break;
        }
    }
    if(flag==false){
        %>
        <script>
        alert("User Email isn't registered..give the valid registered email..!");
        window.location.href = "loginotp.jsp";
        </script>
        <%
    }
    else if(flag==true){
    session.setAttribute("thema",to);
    final String subject = "OTP Generation";
    String messg="Dear User, \r\n";
    
    messg+="\t\t\tBelow is your OTP passcode,\r\n ";
    ...
    Session mailSession = Session.getInstance(props, new javax.mail.Authenticator() {
        ...
        messg+="\r\n";
        messg+="\r\n";
        messg+="\tWe're here to help if you need it, Use this OTP to login.\n";
        messg+="\t\t\t\t\t Thank you ";
...
String s="update customer set otp=? where email='"+to+"'";
PreparedStatement stmt=null;
stmt = con.prepareStatement(s);
        stmt.setString(1,o);
        stmt.executeUpdate();
        
    response.sendRedirect("otpgene.jsp");
    }
    else{}
%>
